  ...
  for (let index = 0; index < lines.length; ) {
    ...
    if (/^[-*+]\s+/.test(line)) {
      ...
      while (
        ...
        items.push(
          stripInlineMarkdown(
            lines[index].trimStart().replace(/^[-*+]\s+/, ""),
          ),
        );
        index += 1;
      }
    ...
    if (/^\d+\.\s+/.test(line)) {
      ...
      while (
        ...
        items.push(
          stripInlineMarkdown(
            lines[index].trimStart().replace(/^\d+\.\s+/, ""),
          ),
        );
        index += 1;
      }
...
function buildSections(markdown: string, profile?: ResumeProfile) {
  ...
  if (
    profile?.summary &&
    !normalizedTitles.has("summary") &&
    !normalizedTitles.has("profile")
  ) {
    output.push({
      title: "Summary",
      blocks: [
        { type: "paragraph", lines: [stripInlineMarkdown(profile.summary)] },
      ],
    });
  }
  ...
  if (
    profile?.skills &&
    !normalizedTitles.has("skills") &&
    !normalizedTitles.has("strengths")
  ) {
    output.push({
      title: "Skills",
      blocks: [
        { type: "unordered-list", items: splitSkillItems(profile.skills) },
      ],
    });
  }
